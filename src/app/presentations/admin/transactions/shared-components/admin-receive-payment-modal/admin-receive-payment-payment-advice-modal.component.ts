...
export class AdminReceivePaymentPaymentAdviceModalComponent implements OnInit {
    ...
    constructor(
        ...
        this.paymentAdviceDataService.GetPenaltySettings(admId).subscribe((res) => {
            this.applyPenalty = res.isPenaltyEnabled == 1 ? true : false;
        })
        this.paymentAdvices = this.config.data;
        this.paymentDate = this.paymentAdvices[0].paymentDate
    ...
    async getPendingPas() {
        this.paymentAdviceDataService.GetAllPendingAdvicesByTenantAndLease(this.paymentAdvices[0].leaseAgreement.tenantId, this.paymentAdvices[0].leaseAgreement.id)
            .subscribe((res) => {
                ...
                const penaltyObservables = this.pendingPaymentAdvices
                    ...
                    .map((item) =>
                        this.paymentAdviceDataService
                            .GetPenaltyByPaymentAdviceWithDate(item.id, this.paymentDate)
                            // .GetPenaltyByPaymentAdvice(item.id )
                            .pipe(
                                map((penalty) => {
                                    console.log("Payment date for penalty:", item);
                ...
                forkJoin(penaltyObservables).subscribe((results) => {
                    ...
                    this.receivePaymentForm.controls['amount'].setValue(this.getTotalDueForSelectedPA());
                    this.receivePaymentForm.controls['mode'].setValue(this.paymentModes[2]);
                    this.receivePaymentForm.controls['refNo'].setValue("NA");
    ...
    async ngOnInit() {
        ...
        this.applyPenalty = penalySettings.isPenaltyEnabled == 1 ? true : false;
        console.log("applyPenaltySetting", this.applyPenalty);
        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                const data: MultiPA_PaymentReceiveDTO = {
                    ...
                    isVerified: true,
                    tenantId: this.selectedPaymentAdvices[0].leaseAgreement.tenantId,
    ...
    confirmWriteOffPenalty(item: PaymentAdviceDto) {
        this.confirmationService.confirm({
            target: event.target as EventTarget,
            message: 'Write off penalty for advice #' + item.id + '?',
            header: 'Confirmation',
            icon: 'pi pi-exclamation-triangle',
            acceptIcon: 'none',
            rejectIcon: 'none',
